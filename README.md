...
>[!IMPORTANT]
> 이 설정은 Atlas OS 를 이용합니다. 이 글을 적고 있는 필자의 경우, 유지 보수등 전반적인 경우 문제가 생기지 않았습니다. 다만, 모든 사람이 문제가 없을거라는 보장은 없기에, 안하셔도 큰 상관은 없습니다. 확실히 설정을 하면, 끊긴다는 느낌과 프레임이 증가하는 효과는 있습니다.

>[!TIP]
> 윈도우 트윅은, 운영체제의 기본 설정을 변경해 선응을 끌어올리거나, 불필요한 기능을 비활성화 시켜 자원을 절약하는 작업입니다.
...
* 불필요한 서비스 비활성화
* 시작 프로그램 최적화
* 레지스트리 설정 등을 통한 응답성 향상 및 지연 시간 최소화

>[!TIP]
> Atlas OS 는 게이밍 및 저사양 PC 를 최적화로 목적으로 만든 windows 의 커스텀 버전입니다. windows 기반으로 만들었지만, 순정 windows 에서 약간의 작업만 설정하는 작업이라고도 합니다.

<h3>주요 설정</h3>

* windows 의 불필요한 서비스 제거 및 비활성화
* 강제 업데이트 비활성화 및 제거
* 낮은 메모리 사용량
* 낮은 지연 시간
* 게이밍에 최적화됨
...
>[!NOTE]
> atlas os 를 다운로드 하기에 부담이 되시는 분들은 아래 항목에서 트윅, 불필요한 앱이 이미 삭제되어있는 윈도우 운영체제를 다운로드 하실수 있습니다. 다만, 토렌트를 필요, iso 파일을 다운로드 하기 떄문에, 최소 8GB 이상의 USB 를 필요로 합니다.
* [홍차의 꿈](https://jsb000.tistory.com)


>[!TIP]
> 추가로, startallback 이라는 프로그램이 있습니다. 기존 윈도우 11의 메뉴바가 불편하신 분들을 위한 도구입니다. 전체적인 UI가 윈도우 7과 같지만, 여러 메뉴로 쉽게 이동할수 있는 등 접근성이 편리한 프로그램입니다.

#### Everything

>[!NOTE]
> 현재 이 프로그램은 컴퓨터를 잘 아는 사람들에게는 가장 유명한 도구입니다. 이 프로그램을 이용한 검색 속도가 다른 도구들보다 월등히 빠른 편이며, 프로그램을 키자 마자 사용할수는 없지만, 약간의 시간이 지나면 검색 하는 그 즉시 실시간으로 파일을 확인할수 있습니다.

* [다운로드 링크](https://www.voidtools.com/ko-kr/)
<br>

> 아마도 이 글을 읽고 있는 분들은 윈도우 11 64bit, 아니면 윈도우 10 64bit 를 사용하시는 사용자분들이 대부분일것 이라고 추측됩니다. 그러기에, '64비트 설치 파일' 항목을 눌러 다운로드 하신후, 실행해주시면 됩니다.